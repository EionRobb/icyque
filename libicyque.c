...
#define ICQ_EVENTS                                                             \
  "myInfo,presence,buddylist,typing,hiddenChat,hist,mchat,sentIM,imState,"     \
  "dataIM,offlineIM,userAddedToBuddyList,service,lifestream,apps,permitDeny,"  \
  "replace,diff" // webrtcMsg
#define ICQ_PRESENCE_FIELDS                                                    \
  "quiet,ssl,abFriendly,role,capabilities,role,abPhones,aimId,autoAddition,"   \
  "friendly,largeIconId,lastseen,mute,pending,state,eventType,seqNum,"         \
  "displayId,friendlyName,userType,statusMsg,statusTime,buddyIcon,"            \
  "abContactName,abPhones,official"
#define ICQ_ASSERT_CAPS                                                        \
  "094613564C7F11D18222444553540000,0946135A4C7F11D18222444553540000,"         \
  "0946135B4C7F11D18222444553540000,0946135D4C7F11D18222444553540000,"         \
  "0946135C4C7F11D18222444553540000,094613574C7F11D18222444553540000,"         \
  "094613504C7F11D18222444553540000,094613514C7F11D18222444553540000,"         \
  "094613534C7F11D18222444553540000,0946135E4C7F11D18222444553540000,"         \
  "094613544C7F11D18222444553540000,0946135F4C7F11D18222444553540000"
...
#define json_object_get_int_member(JSON_OBJECT, MEMBER)                        \
  (json_object_has_member(JSON_OBJECT, MEMBER)                                 \
       ? json_object_get_int_member(JSON_OBJECT, MEMBER)                       \
       : 0)
#define json_object_get_string_member(JSON_OBJECT, MEMBER)                     \
  (json_object_has_member(JSON_OBJECT, MEMBER)                                 \
       ? json_object_get_string_member(JSON_OBJECT, MEMBER)                    \
       : NULL)
#define json_object_get_array_member(JSON_OBJECT, MEMBER)                      \
  (json_object_has_member(JSON_OBJECT, MEMBER)                                 \
       ? json_object_get_array_member(JSON_OBJECT, MEMBER)                     \
       : NULL)
#define json_object_get_object_member(JSON_OBJECT, MEMBER)                     \
  (json_object_has_member(JSON_OBJECT, MEMBER)                                 \
       ? json_object_get_object_member(JSON_OBJECT, MEMBER)                    \
       : NULL)
#define json_object_get_boolean_member(JSON_OBJECT, MEMBER)                    \
  (json_object_has_member(JSON_OBJECT, MEMBER)                                 \
       ? json_object_get_boolean_member(JSON_OBJECT, MEMBER)                   \
       : FALSE)
...
typedef void (*IcyQueProxyCallbackFunc)(IcyQueAccount *ia, JsonObject *obj,
                                        gpointer user_data);
...
static int gc_hmac_sha256(const void *key, size_t keylen, const void *in,
...
gchar *icq_generate_signature(const gchar *data, const gchar *session) {
...
gchar *icq_get_url_sign(IcyQueAccount *ia, gboolean is_post, const gchar *url,
                        const gchar *data) {
...
static JsonObject *icq_generate_robusto_request(IcyQueAccount *ia,
                                                const gchar *method,
                                                JsonObject *params) {
  ...
  g_string_append_printf(request_id, "%lu-%ld", ia->robusto_request_id++,
                         time(NULL) - ia->server_time_offset);
  json_object_set_string_member(robustoRequest, "reqId", request_id->str);
  g_string_free(request_id, TRUE);

  if (ia->robusto_client_id >= 0) {
    json_object_set_int_member(robustoRequest, "clientId",
                               ia->robusto_client_id);
  }
...
static void icq_update_cookies(IcyQueAccount *ia, const GList *cookie_headers) {
...
static void icq_cookie_foreach_cb(gchar *cookie_name, gchar *cookie_value,
                                  GString *str) {
  g_string_append_printf(str, "%s=%s;", cookie_name, cookie_value);
}

static gchar *icq_cookies_to_string(IcyQueAccount *ia) {
  GString *str;
...
static void icq_response_callback(PurpleHttpConnection *http_conn,
  ...
  icq_update_cookies(conn->ia, purple_http_response_get_headers_by_name(
                                   response, "Set-Cookie"));
  ...
  if (body == NULL && error_message != NULL) {
    ...
    gchar *error_msg_formatted =
        g_strdup_printf(_("Connection error: %s."), error_message);
    purple_connection_error(conn->ia->pc, PURPLE_CONNECTION_ERROR_NETWORK_ERROR,
                            error_msg_formatted);
  ...
  if (body != NULL &&
      !json_parser_load_from_data(parser, body, body_len, NULL)) {
    if (conn->callback) {
...
static void icq_fetch_url_with_method(IcyQueAccount *ia, const gchar *method,
                                      const gchar *url, const gchar *postdata,
                                      IcyQueProxyCallbackFunc callback,
                                      gpointer user_data) {
  PurpleAccount *account;
...
static PurpleGroup *icq_get_or_create_default_group(const gchar *group_name) {
  if (group_name == NULL) {
    group_name = "ICQ";
  }
...
static const char *icq_list_icon(PurpleAccount *account, PurpleBuddy *buddy) {
  return "icq";
}

static GList *icq_status_types(PurpleAccount *account) {
  GList *types = NULL;
  PurpleStatusType *status;

  status = purple_status_type_new_with_attrs(
      PURPLE_STATUS_AVAILABLE, "online", _("Online"), TRUE, TRUE, FALSE,
      "message", _("Status"), purple_value_new(PURPLE_TYPE_STRING), NULL);
  types = g_list_append(types, status);

  status = purple_status_type_new_with_attrs(
      PURPLE_STATUS_AWAY, "away", _("Away"), TRUE, FALSE, FALSE, "message",
      _("Status"), purple_value_new(PURPLE_TYPE_STRING), NULL);
  types = g_list_append(types, status);

  status = purple_status_type_new_with_attrs(
      PURPLE_STATUS_MOBILE, "mobile", _("Mobile"), TRUE, FALSE, FALSE,
      "message", _("Status"), purple_value_new(PURPLE_TYPE_STRING), NULL);
  types = g_list_append(types, status);

  status = purple_status_type_new_full(PURPLE_STATUS_OFFLINE, "offline",
...
static gchar *icq_status_text(PurpleBuddy *buddy) {
  const gchar *message = purple_status_get_attr_string(
...
static void icq_tooltip_text(PurpleBuddy *buddy,
  ...
  purple_notify_user_info_add_pair_html(user_info, _("Status"),
...
static void icq_set_status(PurpleAccount *account, PurpleStatus *status) {
  PurpleConnection *pc = purple_account_get_connection(account);
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
  const gchar *status_id =
      purple_status_get_id(status); // online, mobile, away, offline
  ...
  g_string_append_printf(postdata, "ts=%d&",
                         (int)(time(NULL) - ia->server_time_offset));
  ...
  g_string_append_printf(postdata, "&sig_sha256=%s",
                         purple_url_encode(sig_sha256));
  g_free(sig_sha256);

  icq_fetch_url_with_method(ia, "POST", url, postdata->str, NULL /*TODO*/,
                            NULL);
  ...
  g_string_append(postdata, "f=json&");
  g_string_append_printf(postdata, "nonce=%s&", purple_url_encode(uuid));
  g_string_append_printf(postdata, "statusMsg=%s&",
                         purple_url_encode(message ? message : ""));
  g_string_append_printf(postdata, "ts=%d",
                         (int)(time(NULL) - ia->server_time_offset));
...
static void icq_block_user(PurpleConnection *pc, const char *who) {
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
...
static void icq_unblock_user(PurpleConnection *pc, const char *who) {
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
  ...
  g_string_append_printf(postdata, "aimsid=%s&", purple_url_encode(ia->aimsid));
  g_string_append(postdata, "f=json&");
  g_string_append_printf(postdata, "pdIgnoreRemove=%s&",
...
static void icq_got_user_info(IcyQueAccount *ia, JsonObject *obj,
                              gpointer user_data) {
  if (obj == NULL) {
    return;
  }
  ...
  purple_notify_user_info_add_pair_html(
      user_info, _("First name"),
      json_object_get_string_member(profile, "firstName"));
  purple_notify_user_info_add_pair_html(
      user_info, _("Last name"),
      json_object_get_string_member(profile, "lastName"));
  purple_notify_user_info_add_pair_html(
      user_info, _("Gender"), json_object_get_string_member(profile, "gender"));
  purple_notify_user_info_add_pair_html(
      user_info, _("Alias"),
...
static void icq_get_info(PurpleConnection *pc, const gchar *who) {
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
  GString *postdata = g_string_new(NULL);
  ...
  icq_fetch_url_with_method(ia, "POST", url, postdata->str, icq_got_user_info,
                            NULL);
...
static void icq_add_buddy_with_invite(PurpleConnection *pc, PurpleBuddy *buddy,
  ...
  g_string_append_printf(postdata, "a=%s&", purple_url_encode(ia->token));
  g_string_append_printf(postdata, "aimsid=%s&", purple_url_encode(ia->aimsid));
  g_string_append_printf(postdata, "authorizationMsg=%s&",
                         purple_url_encode(message));
  ...
  g_string_append_printf(postdata, "nick=%s&",
                         purple_url_encode(alias && *alias ? alias : who));
  g_string_append_printf(postdata, "nonce=%s&", purple_url_encode(uuid));
...
#if !PURPLE_VERSION_CHECK(3, 0, 0)
static void icq_add_buddy(PurpleConnection *pc, PurpleBuddy *buddy,
                          PurpleGroup *group) {
  icq_add_buddy_with_invite(pc, buddy, group, NULL);
}
#endif

static void icq_remove_buddy_by_name(IcyQueAccount *ia, const gchar *who) {
  GString *postdata = g_string_new(NULL);
...
// static void
// icq_remove_buddy(PurpleConnection *connection, PurpleBuddy *buddy,
// PurpleGroup *group)
// {
// IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
...
static void icq_chat_leave(PurpleConnection *pc, int id) {
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
  const gchar *sn;
...
static void icq_chat_kick(PurpleConnection *pc, int id, const gchar *who) {
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
  const gchar *sn;
  PurpleChatConversation *chatconv;
...
static GList *icq_chat_info(PurpleConnection *pc) {
  GList *m = NULL;
...
static GHashTable *icq_chat_info_defaults(PurpleConnection *pc,
                                          const char *chatname) {
  GHashTable *defaults =
...
static gchar *icq_get_chat_name(GHashTable *data) {
  gchar *temp;
...
static void icq_chat_invite(PurpleConnection *pc, int id, const char *message,
                            const char *who) {
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
  GString *postdata = g_string_new(NULL);
  gchar *uuid = purple_uuid_random();
  ...
  const gchar *sn =
      purple_conversation_get_data(PURPLE_CONVERSATION(chatconv), "sn");
...
static void icq_join_chat_got_chat_info_cb(IcyQueAccount *ia, JsonObject *data,
                                           gpointer user_data) {
  // Example response:
  /*
          {
  ...
  */
  ...
  PurpleChatConversation *chatconv =
      purple_serv_got_joined_chat(ia->pc, g_str_hash(chat_sn), chat_sn);
  purple_conversation_set_data(PURPLE_CONVERSATION(chatconv), "sn",
                               g_strdup(chat_sn));
  purple_conversation_set_data(PURPLE_CONVERSATION(chatconv), "stamp",
                               g_strdup(chat_stamp));
  purple_conversation_set_title(PURPLE_CONVERSATION(chatconv),
                                g_strdup(chat_name));
...
static void icq_joined_chat_cb(IcyQueAccount *ia, JsonObject *data,
                               gpointer user_data) {
  ...
  JsonObject *getIdInfoRequest =
      icq_generate_robusto_request(ia, "getIdInfo", getIdInfoParams);

  gchar *getIdInfoRequestStr = json_object_to_string(getIdInfoRequest);
  json_object_unref(getIdInfoRequest);
  g_free(chatId);

  icq_fetch_url_with_method(ia, "POST", ICQ_RAPI_SERVER, getIdInfoRequestStr,
                            icq_join_chat_got_chat_info_cb, NULL);
  g_free(getIdInfoRequestStr);
}

static int icq_join_chat_send_request(IcyQueAccount *ia, const gchar *stamp) {
  ...
  JsonObject *joinChatRequest =
      icq_generate_robusto_request(ia, "joinChat", joinChatParams);

  gchar *joinChatRequestStr = json_object_to_string(joinChatRequest);
  json_object_unref(joinChatRequest);

  icq_fetch_url_with_method(ia, "POST", ICQ_RAPI_SERVER, joinChatRequestStr,
                            icq_joined_chat_cb, g_strdup(stamp));
  g_free(joinChatRequestStr);
}

static void icq_join_chat_with_sn_convert_request_cb(IcyQueAccount *ia,
                                                     JsonObject *data,
                                                     gpointer user_data) {
  ...
  */
  ...
  JsonObject *json_results = json_object_get_object_member(data, "results");
  const gchar *chat_stamp =
      json_object_get_string_member(json_results, "stamp");

  icq_join_chat_send_request(ia, chat_stamp);
}

static void icq_join_chat_with_sn_send_convert_request(IcyQueAccount *ia,
                                                       const gchar *sn) {
  ...
  */
  ...
  JsonObject *getChatInfoRequest =
      icq_generate_robusto_request(ia, "getChatInfo", getChatInfoParams);

  gchar *getChatInfoRequestStr = json_object_to_string(getChatInfoRequest);
  json_object_unref(getChatInfoRequest);

  icq_fetch_url_with_method(ia, "POST", ICQ_RAPI_SERVER, getChatInfoRequestStr,
                            icq_join_chat_with_sn_convert_request_cb, NULL);
  g_free(getChatInfoRequestStr);
}

static void icq_join_chat(PurpleConnection *pc, GHashTable *data) {
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
  const gchar *sn;
  PurpleChatConversation *chatconv;
...
static void icq_sent_msg(IcyQueAccount *ia, JsonObject *obj,
                         gpointer user_data) {
  JsonObject *response = json_object_get_object_member(obj, "response");
  JsonObject *data = json_object_get_object_member(response, "data");
...
static int icq_send_msg(IcyQueAccount *ia, const gchar *to,
                        const gchar *message) {
  GString *postdata = g_string_new(NULL);
...
static int icq_send_im(PurpleConnection *pc,
#if PURPLE_VERSION_CHECK(3, 0, 0)
                       PurpleMessage *msg) {
...
static gint icq_chat_send(PurpleConnection *pc, gint id,
#if PURPLE_VERSION_CHECK(3, 0, 0)
                          PurpleMessage *msg) {
  const gchar *message = purple_message_get_contents(msg);
#else
...
static guint icq_send_typing(PurpleConnection *pc, const gchar *who,
...
static void icq_got_buddy_icon(IcyQueAccount *ia, JsonObject *obj,
                               gpointer user_data) {
  ...
  if (obj != NULL) {
    ...
    purple_buddy_icons_set_for_user(ia->account, username, response_dup,
                                    response_len, buddyIcon);
  }

  g_dataset_destroy(buddy);
}

static void icq_get_chat_history(IcyQueAccount *ia, const gchar *chatId,
                                 const gchar *fromMsg, gint64 count,
                                 IcyQueProxyCallbackFunc callback,
                                 gpointer user_data) {
  ...
  // TODO: the initial histDlgState event contains a patchVersion. Store this
  // per conversation and use that..?
  ...
  JsonObject *getHistoryRequest =
      icq_generate_robusto_request(ia, "getHistory", getHistoryParams);
  gchar *getHistoryRequestStr = json_object_to_string(getHistoryRequest);
  json_object_unref(getHistoryRequest);

  icq_fetch_url_with_method(ia, "POST", ICQ_RAPI_SERVER, getHistoryRequestStr,
                            callback, user_data);
  g_free(getHistoryRequestStr);
}

static GList *valid_icyque_accounts = NULL;
#define ICYQUE_ACCOUNT_IS_VALID(ia)                                            \
  (g_list_find(valid_icyque_accounts, (ia)) != NULL)

static void icq_mark_message_as_read(IcyQueAccount *ia, const gchar *sn,
  ...
  JsonObject *setDlgStateRequest =
      icq_generate_robusto_request(ia, "setDlgState", setDlgStateParams);
  gchar *setDlgStateRequestStr = json_object_to_string(setDlgStateRequest);
  json_object_unref(setDlgStateRequest);

  icq_fetch_url_with_method(ia, "POST", ICQ_RAPI_SERVER, setDlgStateRequestStr,
                            NULL, NULL);
  g_free(setDlgStateRequestStr);
}

static void icq_unread_message_load_cb(IcyQueAccount *ia, JsonObject *data,
                                       gpointer user_data) {
  ...
  if (status && json_object_get_int_member(status, "code") == 20000) {
    ...
    for (i = (len - 1); i >= 0; i--) {
      ...
      purple_serv_got_im(ia->pc, sn, escaped_text, PURPLE_MESSAGE_RECV,
                         (time_t)time);
      g_free(escaped_text);
    }
    ...
    if (len > 0) {
      ...
      const gchar *messageId =
          json_object_get_string_member(lastMessage, "msgId");
      gint64 time = json_object_get_int_member(lastMessage, "time");

      ia->last_message_timestamp = MAX(ia->last_message_timestamp, time);
      purple_account_set_int(ia->account, "last_message_timestamp_high",
                             ia->last_message_timestamp >> 32);
      purple_account_set_int(ia->account, "last_message_timestamp_low",
                             ia->last_message_timestamp & 0xFFFFFFFF);
      ...
      // .. Should it be done at all? (Responding to a message automatically
      // marks received messages as read)
      // TODO: Whatever the decision: This should be applied to normal messages
      // (online messages) as well.
...
static void icq_process_event(IcyQueAccount *ia, const gchar *event_type,
                              JsonObject *data) {
  if (event_type == NULL)
    return;
  if (!ICYQUE_ACCOUNT_IS_VALID(ia))
    return;
  ...
  if (purple_strequal(event_type, "presence")) {
    ...
    if (statusMsg != NULL) {
      purple_protocol_got_user_status(ia->account, aimId, state, "message",
                                      statusMsg, NULL);
    } else {
    ...
    if (pbuddy != NULL) {
      ...
      if (!purple_strequal(purple_buddy_icons_get_checksum_for_user(pbuddy),
                           buddyIcon)) {
        g_dataset_set_data_full(pbuddy, "buddyIcon", g_strdup(buddyIcon),
                                g_free);

        icq_fetch_url_with_method(ia, "GET", buddyIcon, NULL,
                                  icq_got_buddy_icon, pbuddy);
      }
    }

  } else if (purple_strequal(event_type, "typing")) {
    const gchar *aimId = json_object_get_string_member(data, "aimId");
    const gchar *typingStatus =
        json_object_get_string_member(data, "typingStatus");
    ...
    if (json_object_get_boolean_member(data, "starting")) {
      ...
      if (unreadMsgCnt > 0) {
        ...
        // TODO: "fromMsgId == -1" means last message. So the following loads
        // all messages that are unread.
        // Should we instead store the last message id that we saw, and sync all
        // messages that have been sent in the meantime (with other clients
        // e.g.) ?
        // FIXME: Do NOT use "-1" here. This might lead to a race condition,
        // when another client sends a new message now.
        icq_get_chat_history(ia, sn, "-1", -unreadMsgCnt,
                             icq_unread_message_load_cb, NULL);
      }
    } else {
      JsonObject *tail = json_object_get_object_member(data, "tail");
      JsonArray *messages = json_object_get_array_member(
          (tail != NULL ? tail : data), "messages");
      ...
      for (i = 0; i < len; i++) {
        ...
        if (ia->last_message_timestamp && time > ia->last_message_timestamp) {
          const gchar *mediaType =
              json_object_get_string_member(message, "mediaType");
          const gchar *text = json_object_get_string_member(message, "text");
          ...
          if (g_str_has_suffix(sn, "@chat.agent")) {
            ...
            JsonObject *memberEvent =
                json_object_get_object_member(chat, "memberEvent");

            if (memberEvent != NULL) {
              PurpleChatConversation *chatconv =
                  purple_conversations_find_chat_with_account(sn, ia->account);
              const gchar *memberEventType =
                  json_object_get_string_member(memberEvent, "type");
              if (purple_strequal(memberEventType, "invite") ||
                  purple_strequal(memberEventType, "addMembers")) {
                JsonArray *members =
                    json_object_get_array_member(memberEvent, "members");
                // add members to the group chat
                const gchar *role =
                    json_object_get_string_member(memberEvent, "role");
                ...
                purple_chat_conversation_add_users(chatconv, users, NULL, flags,
                                                   TRUE);
                while (users != NULL) {
                  g_free(users->data);
              ...
              if (chatconv == NULL) {
                chatconv =
                    purple_serv_got_joined_chat(ia->pc, g_str_hash(sn), sn);
                purple_conversation_set_data(PURPLE_CONVERSATION(chatconv),
                                             "sn", g_strdup(sn));
                purple_chat_conversation_set_topic(chatconv, NULL, chatName);
              }

              purple_serv_got_chat_in(ia->pc, g_str_hash(sn), sender, msg_flags,
                                      escaped_text, time);

            } else {
              purple_debug_warning(
                  "icyque", "Unknown chat message mediaType '%s'\n", mediaType);
            }
            ...
            if (purple_strequal(mediaType, "text")) {
              if (msg_flags & PURPLE_MESSAGE_SEND) {
                PurpleIMConversation *imconv =
                    purple_conversations_find_im_with_account(sn, ia->account);
                PurpleMessage *msgObj =
                    purple_message_new_outgoing(sn, escaped_text, msg_flags);
                if (imconv == NULL) {
                  imconv = purple_im_conversation_new(ia->account, sn);
                }
                purple_message_set_time(msgObj, time);
                purple_conversation_write_message(PURPLE_CONVERSATION(imconv),
                                                  msgObj);

              } else {
                purple_serv_got_im(ia->pc, sn, escaped_text, msg_flags,
                                   (time_t)time);
              }
            } else {
              purple_debug_warning(
                  "icyque", "Unknown IM message mediaType '%s'\n", mediaType);
            }
          }
    ...
    for (i = 0; i < len; i++) {
      ...
      for (j = 0; j < buddies_len; j++) {
        ...
        const gchar *statusMsg =
            json_object_get_string_member(buddy, "statusMsg");
        ...
        if (g_str_has_suffix(aimId, "@chat.agent")) {
          ...
          if (chat == NULL) {
            const gchar *friendly =
                json_object_get_string_member(buddy, "friendly");
            chat = purple_chat_new(ia->account, friendly,
                                   icq_chat_info_defaults(ia->pc, aimId));
          ...
          if (statusMsg != NULL) {
            purple_protocol_got_user_status(ia->account, aimId, state,
...
static void icq_fetch_events_cb(IcyQueAccount *ia, JsonObject *obj,
                                gpointer user_data) {
  if (!ICYQUE_ACCOUNT_IS_VALID(ia))
    return;

  if (obj == NULL) {
    icq_fetch_url_with_method(ia, "GET", ia->last_fetchBaseURL, NULL,
                              icq_fetch_events_cb, NULL);
    return;
  }
  ...
  const gchar *fetchBaseURL =
      json_object_get_string_member(data, "fetchBaseURL");
  ...
  icq_fetch_url_with_method(ia, "GET", fetchBaseURL, NULL, icq_fetch_events_cb,
                            NULL);
}

static void icq_robusto_add_client_cb(IcyQueAccount *ia, JsonObject *obj,
                                      gpointer user_data) {
  JsonObject *status = json_object_get_object_member(obj, "status");
  if (!status || json_object_get_int_member(status, "code") != 20000) {
    purple_connection_error(ia->pc,
                            PURPLE_CONNECTION_ERROR_AUTHENTICATION_FAILED,
                            "Failed to register client at robusto API.");
  } else {
    ...
    purple_debug_info("icyque",
                      "Authentication succeeded. Starting fetch-loop.\n");
    icq_fetch_url_with_method(ia, "GET", ia->last_fetchBaseURL, NULL,
                              icq_fetch_events_cb, NULL);
  }
}
...
static void icq_robusto_add_client(IcyQueAccount *ia) {
  ...
  json_object_set_object_member(addClientParams, "ua",
                                addClientParamsUserAgent);
  json_object_set_string_member(
      addClientParamsUserAgent, "app",
      "icq"); // TODO pretend to be official client or Pidgin?
  ...
  JsonObject *addClientRequest =
      icq_generate_robusto_request(ia, "addClient", addClientParams);
  ...
  icq_fetch_url_with_method(ia, "POST", ICQ_RAPI_SERVER, addClientRequestStr,
                            icq_robusto_add_client_cb, NULL);
}

static void icq_robusto_gen_token_cb(IcyQueAccount *ia, JsonObject *obj,
                                     gpointer user_data) {
  JsonObject *results = json_object_get_object_member(obj, "results");
  ...
  if (authToken == NULL) {
    purple_connection_error(
        ia->pc, PURPLE_CONNECTION_ERROR_AUTHENTICATION_FAILED,
...
static void icq_robusto_gen_token(IcyQueAccount *ia) {
  ...
  icq_fetch_url_with_method(ia, "POST", url, postdata->str,
                            icq_robusto_gen_token_cb, NULL);
  g_string_free(postdata, TRUE);
}

static void icq_session_start_cb(IcyQueAccount *ia, JsonObject *obj,
                                 gpointer user_data) {
  if (!ICYQUE_ACCOUNT_IS_VALID(ia))
    return;
...
static void icq_session_start(IcyQueAccount *ia) {
  if (!ICYQUE_ACCOUNT_IS_VALID(ia))
    return;
  ...
  // Make sure these are added alphabetically for the signature to work
  g_string_append_printf(postdata, "a=%s&", purple_url_encode(ia->token));
  // g_string_append_printf(postdata, "assertCaps=%s&",
  // purple_url_encode(ICQ_ASSERT_CAPS)); g_string_append_printf(postdata,
  // "buildNumber=%s&", purple_url_encode("23341"));
  // g_string_append_printf(postdata, "clientName=%s&",
  // purple_url_encode("ICQ"));
  g_string_append_printf(postdata, "deviceId=%s&",
                         purple_url_encode(ia->device_id));
  g_string_append_printf(postdata, "events=%s&", purple_url_encode(ICQ_EVENTS));
  g_string_append(postdata, "f=json&");
  g_string_append(postdata, "imf=plain&");
  g_string_append_printf(postdata, "includePresenceFields=%s&",
                         purple_url_encode(ICQ_PRESENCE_FIELDS));
  // g_string_append_printf(postdata, "interestCaps=%s&",
  // purple_url_encode("094613504C7F11D18222444553540000,094613514C7F11D18222444553540000,8EEC67CE70D041009409A7C1602A5C84"));
  ...
  icq_fetch_url_with_method(ia, "POST", url, postdata->str,
                            icq_session_start_cb, NULL);

  g_string_free(postdata, TRUE);
}

static void icq_login_cb(IcyQueAccount *ia, JsonObject *obj,
                         gpointer user_data) {
  JsonObject *response = json_object_get_object_member(obj, "response");
  ...
  if (json_object_get_int_member(response, "statusCode") == 200) {
    ...
    const gchar *sessionSecret =
        json_object_get_string_member(data, "sessionSecret");
    const gint64 hostTime = json_object_get_int_member(data, "hostTime");

    if (a != NULL) {
      ia->token = g_strdup(purple_url_decode(a));
      ia->session_key = icq_generate_signature(
          sessionSecret, purple_connection_get_password(ia->pc));
      ...
      purple_account_set_int(ia->account, "server_time_offset",
                             ia->server_time_offset);
  ...
  purple_connection_error(ia->pc, PURPLE_CONNECTION_ERROR_AUTHENTICATION_FAILED,
                          _("Bad username/password"));
}

static void icq_mfa_text_entry_cb(IcyQueAccount *ia, JsonObject *obj,
                                  gpointer user_data) {
  JsonObject *response = json_object_get_object_member(obj, "response");
...
static void icq_mfa_text_entry(gpointer user_data, const gchar *code) {
  IcyQueAccount *ia = user_data;
  const gchar *username = purple_account_get_username(ia->account);
  GString *postdata = g_string_new(NULL);

  g_string_append_printf(postdata, "msisdn=%s&",
                         purple_url_encode(&username[1]));
  g_string_append_printf(postdata, "trans_id=%s&",
                         purple_url_encode(ia->sms_trans_id));
  ...
  icq_fetch_url_with_method(ia, "POST",
                            "https://u.icq.net/smsreg/loginWithPhoneNumber.php",
                            postdata->str, icq_mfa_text_entry_cb, NULL);
...
static void icq_mfa_cancel(gpointer user_data) {
  IcyQueAccount *ia = user_data;

  purple_connection_error(ia->pc, PURPLE_CONNECTION_ERROR_AUTHENTICATION_FAILED,
                          _("Cancelled 2FA auth"));
}

static void icq_sms_login_cb(IcyQueAccount *ia, JsonObject *obj,
                             gpointer user_data) {
  JsonObject *response = json_object_get_object_member(obj, "response");
  ...
  if (json_object_get_int_member(response, "statusCode") == 200) {
    ...
    purple_request_input(
        ia->pc, _("Two-factor authentication"), _("Enter SMS code"),
        _("You will be sent an SMS message containing your auth code."), NULL,
        FALSE, FALSE, "", _("_Login"), G_CALLBACK(icq_mfa_text_entry),
        _("_Cancel"), G_CALLBACK(icq_mfa_cancel),
        purple_request_cpar_from_connection(ia->pc), ia);
  }
}

static void icq_login(PurpleAccount *account) {
  IcyQueAccount *ia;
  PurpleConnection *pc = purple_account_get_connection(account);
  ...
  ia->sent_messages_hash =
      g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  ia->device_id =
      g_strdup(purple_account_get_string(ia->account, "device_id", NULL));
  ia->keepalive_pool = purple_http_keepalive_pool_new();
  ia->token = g_strdup(purple_account_get_string(ia->account, "token", NULL));
  ia->session_key =
      g_strdup(purple_account_get_string(ia->account, "session_key", NULL));
  ia->server_time_offset =
      purple_account_get_int(ia->account, "server_time_offset", 0);
  ia->robusto_client_id = -1;

  if (ia->device_id == NULL) {
    // TODO pretend to be official client or Pidgin?
    ia->device_id =
        g_strdup_printf("icq-%08x%08x", g_random_int(), g_random_int());
    purple_account_set_string(ia->account, "device_id", ia->device_id);
  }

  ia->last_message_timestamp =
      purple_account_get_int(account, "last_message_timestamp_high", 0);

  if (ia->last_message_timestamp != 0) {
    ia->last_message_timestamp =
        (ia->last_message_timestamp << 32) |
        ((guint64)purple_account_get_int(account, "last_message_timestamp_low",
                                         0) &
         0xFFFFFFFF);
  }
  ...
  if (ia->token == NULL) {
    ...
    if (username[0] != '+') {
      ...
      g_string_append_printf(postdata, "clientName=%s&",
                             purple_url_encode("ICQ"));
      g_string_append_printf(postdata, "clientVersion=%s&",
                             purple_url_encode("7.4"));
      g_string_append_printf(postdata, "devId=%s&",
                             purple_url_encode(ICQ_DEVID));
      g_string_append(postdata, "f=json&");
      g_string_append(postdata, "idType=ICQ&");
      g_string_append_printf(
          postdata, "pwd=%s&",
          purple_url_encode(purple_connection_get_password(pc)));
      g_string_append_printf(postdata, "s=%s&", purple_url_encode(username));

      icq_fetch_url_with_method(ia, "POST",
                                "https://api.login.icq.net/auth/clientLogin",
                                postdata->str, icq_login_cb, NULL);
      ...
      icq_fetch_url_with_method(
          ia, "POST", "https://u.icq.net/smsreg/requestPhoneValidation.php",
          postdata->str, icq_sms_login_cb, NULL);
    }
...
static void icq_close(PurpleConnection *pc) {
  IcyQueAccount *ia = purple_connection_get_protocol_data(pc);
...
static PurpleCmdRet icq_cmd_leave(PurpleConversation *conv, const gchar *cmd,
...
static PurpleCmdRet icq_cmd_kick(PurpleConversation *conv, const gchar *cmd,
                                 gchar **args, gchar **error, void *data) {
  PurpleConnection *pc = NULL;
  int id = -1;
...
static gboolean plugin_load(PurplePlugin *plugin, GError **error) {
  purple_cmd_register("leave", "", PURPLE_CMD_P_PLUGIN,
...
static void plugin_init(PurplePlugin *plugin) {
  ...
  // icyque_add_account_options(prpl_info->protocol_options);
...
static PurplePluginInfo info = {PURPLE_PLUGIN_MAGIC,
                                ...
                                */
                                2, 1, PURPLE_PLUGIN_PROTOCOL, /* type */
                                NULL,                    /* ui_requirement */
                                0,                       /* flags */
                                NULL,                    /* dependencies */
...
#define ICYQUE_PROTOCOL(obj)                                                   \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), ICYQUE_TYPE_PROTOCOL, IcyQueProtocol))
#define ICYQUE_PROTOCOL_CLASS(klass)                                           \
  (G_TYPE_CHECK_CLASS_CAST((klass), ICYQUE_TYPE_PROTOCOL, IcyQueProtocolClass))
#define ICYQUE_IS_PROTOCOL(obj)                                                \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj), ICYQUE_TYPE_PROTOCOL))
#define ICYQUE_IS_PROTOCOL_CLASS(klass)                                        \
  (G_TYPE_CHECK_CLASS_TYPE((klass), ICYQUE_TYPE_PROTOCOL))
#define ICYQUE_PROTOCOL_GET_CLASS(obj)                                         \
  (G_TYPE_INSTANCE_GET_CLASS((obj), ICYQUE_TYPE_PROTOCOL, IcyQueProtocolClass))
...
static void icyque_protocol_init(PurpleProtocol *prpl_info) {
...
static void icyque_protocol_class_init(PurpleProtocolClass *prpl_info) {
  prpl_info->login = icq_login;
  prpl_info->close = icq_close;
  prpl_info->status_types = icq_status_types;
  prpl_info->list_icon = icq_list_icon;
...
static void icyque_protocol_im_iface_init(PurpleProtocolIMIface *prpl_info) {
            ...
            PURPLE_IMPLEMENT_INTERFACE_STATIC(PURPLE_TYPE_PROTOCOL_CLIENT_IFACE,
                ...
                PURPLE_IMPLEMENT_INTERFACE_STATIC(
                    ...
                    PURPLE_IMPLEMENT_INTERFACE_STATIC(
                        ...
                        PURPLE_IMPLEMENT_INTERFACE_STATIC(
                            PURPLE_TYPE_PROTOCOL_MEDIA_IFACE,
                            icyque_protocol_media_iface_init)

                            PURPLE_IMPLEMENT_INTERFACE_STATIC(
                                PURPLE_TYPE_PROTOCOL_ROOMLIST_IFACE,
...
static PurplePluginInfo *plugin_query(GError **error) {
  return purple_plugin_info_new(
      "id", "prpl-eionrobb-icyque", "name", "ICQ (WIM)", "version", "0.1",
      "category", N_("Protocol"), "summary", N_("ICQ-WIM Protocol Plugin."),
      "description", N_("Adds ICQ protocol support to libpurple."), "website",
      "https://github.com/EionRobb/icyque/", "abi-version", PURPLE_ABI_VERSION,
      "flags",
      PURPLE_PLUGIN_INFO_FLAGS_INTERNAL | PURPLE_PLUGIN_INFO_FLAGS_AUTO_LOAD,
      NULL);
}

PURPLE_PLUGIN_INIT(icyque, plugin_query, libpurple3_plugin_load,
                   libpurple3_plugin_unload);
